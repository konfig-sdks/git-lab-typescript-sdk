...
import { readableStreamToString, GitLabError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new GitLabError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new GitLabError(e, e.response?.data)
                }
                if (innerError instanceof GitLabError) {
                    // Got "GitLabError" from the above try block
                    throw innerError;
                }
